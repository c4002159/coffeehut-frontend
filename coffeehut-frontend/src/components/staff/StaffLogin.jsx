// StaffLogin.jsx — Staff login page -WeiqiWang
...
export default function StaffLogin({ onLoginSuccess }) {
    ...
    // Restore remembered email from localStorage on mount. -WeiqiWang
    useEffect(() => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (rememberMe) {
                localStorage.setItem('rememberedEmail', email); // email only, never password -WeiqiWang
            ...
            <div className="flex-1 px-4 pt-8 pb-12">
                ...
                <form className="space-y-4" onSubmit={handleLogin}>
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="staff@coffeehut.com"
                                className="w-full h-14 pl-12 pr-4 bg-white border border-primary/10 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                                required
                            />
                        </div>
                    </div>

                    {/* Hide Edge's built-in password reveal icon via CSS to avoid duplicate toggle buttons. -WeiqiWang */}
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="••••••••"
                                className="w-full h-14 pl-12 pr-12 bg-white border border-primary/10 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none transition-all [&::-ms-reveal]:hidden [&::-ms-clear]:hidden"
                                required
                            />
                            <button
                                type="button"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-primary text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/20 flex items-center justify-center gap-2 mt-6 active:scale-95 transition-transform"
                    >
